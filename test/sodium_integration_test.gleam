...
pub fn sodium_init_and_shutdown_test() {
  ...
  let assert Ok(_) = test_helpers.with_provider(provider, name, fn(provider, handle) {
    // Check health
...
pub fn sodium_get_handle_after_init_test() {
  ...
  let assert Ok(_) = test_helpers.with_provider(provider, name, fn(_provider, _handle) {
    // Get handle by name
...
pub fn sodium_full_handshake_between_two_providers_test() {
  ...
  let provider_a = sodium_adapter.new()
  let provider_b = sodium_adapter.new()

  let assert Ok(_) = test_helpers.with_provider(provider_a, name_a, fn(provider_a, handle_a) {
    let assert Ok(_) = test_helpers.with_provider(provider_b, name_b, fn(provider_b, handle_b) {
      ...
      let assert Ok(types.Continue(_state_b, Some(msg_b))) =
        { provider_b.handshake_start }(handle_b, node_b, node_a, Some(msg_a))
...
pub fn sodium_encrypt_decrypt_roundtrip_test() {
  ...
  let assert Ok(_) = test_helpers.with_provider(provider_a, name_a, fn(provider_a, handle_a) {
    let assert Ok(_) = test_helpers.with_provider(provider_b, name_b, fn(provider_b, handle_b) {
      ...
      // Get contexts
      let assert Some(ctx_a) = { provider_a.secure_context }(handle_a, node_b)
      let assert Some(ctx_b) = { provider_b.secure_context }(handle_b, node_a)

      // A encrypts message
      let plaintext = <<"Hello, secure world!">>
      let assert Ok(ciphertext) = { provider_a.encrypt }(handle_a, ctx_a, plaintext)

      should.be_true(bit_array.byte_size(ciphertext) > bit_array.byte_size(plaintext))

      // B decrypts message
      let assert Ok(decrypted) = { provider_b.decrypt }(handle_b, ctx_b, ciphertext)
  ...
  let assert Ok(_) = test_helpers.with_provider(provider_a, name_a, fn(provider_a, handle_a) {
    let assert Ok(_) = test_helpers.with_provider(provider_b, name_b, fn(provider_b, handle_b) {
      ...
      list.each(messages, fn(msg) {
        let assert Ok(ciphertext) = { provider_a.encrypt }(handle_a, ctx_a, msg)
        let assert Ok(decrypted) = { provider_b.decrypt }(handle_b, ctx_b, ciphertext)
...
pub fn sodium_decrypt_with_wrong_key_fails_test() {
  ...
  let provider_a = sodium_adapter.new()
  let provider_b = sodium_adapter.new()
  let provider_c = sodium_adapter.new()

  let assert Ok(_) = test_helpers.with_provider(provider_a, name_a, fn(provider_a, handle_a) {
    let assert Ok(_) = test_helpers.with_provider(provider_b, name_b, fn(provider_b, handle_b) {
      let assert Ok(_) = test_helpers.with_provider(provider_c, name_c, fn(provider_c, handle_c) {
        let node_a = "node_a@localhost"
        let node_b = "node_b@localhost"
        ...
        let assert Ok(types.Continue(state_a2, Some(msg_a2))) =
          { provider_a.handshake_start }(handle_a, node_a, node_c, None)
        let assert Ok(types.Continue(_state_c, Some(msg_c))) =
          { provider_c.handshake_start }(handle_c, node_c, node_a, Some(msg_a2))
        let assert Ok(types.Established(_)) =
          { provider_a.handshake_continue }(handle_a, state_a2, msg_c)

        // A encrypts for B
        let assert Some(ctx_a_b) = { provider_a.secure_context }(handle_a, node_b)
        let plaintext = <<"Secret for B">>
        let assert Ok(ciphertext) = { provider_a.encrypt }(handle_a, ctx_a_b, plaintext)

        // C tries to decrypt with wrong key - should fail
        let assert Some(ctx_c) = { provider_c.secure_context }(handle_c, node_a)
        let result = { provider_c.decrypt }(handle_c, ctx_c, ciphertext)
...
pub fn sodium_rekey_changes_key_id_test() {
  ...
  let assert Ok(_) = test_helpers.with_provider(provider_a, name_a, fn(provider_a, handle_a) {
    let assert Ok(_) = test_helpers.with_provider(provider_b, name_b, fn(provider_b, handle_b) {
      ...
      let assert Some(ctx1) = { provider_a.secure_context }(handle_a, node_b)
      let key_id1 = types.context_key_id(ctx1)

      // Rekey
      let assert Ok(_) = { provider_a.rekey }(handle_a, node_b)

      let assert Some(ctx2) = { provider_a.secure_context }(handle_a, node_b)
...
pub fn concurrent_handshakes_test() {
  ...
  let _ = list.each(ids, fn(i) {
    ...
    let _ = process.spawn(fn() {
      ...
      let send_and_shutdown = fn(msg, ha, hb) {
        let _ = process.send(parent, msg)
        let _ = { provider.shutdown }(ha)
        let _ = { provider.shutdown }(hb)
        Nil
      }
          ...
          case { provider.handshake_start }(handle_b, node_b, node_a, Some(msg_a)) {
            Ok(types.Continue(_state_b, Some(msg_b))) -> {
              case { provider.handshake_continue }(handle_a, state_a, msg_b) {
                Ok(types.Established(_ctx_a)) -> send_and_shutdown(<<"ok">>, handle_a, handle_b)
                Ok(types.Continue(_, _)) -> send_and_shutdown(<<"err">>, handle_a, handle_b)
                Ok(types.HandshakeError(_)) -> send_and_shutdown(<<"err">>, handle_a, handle_b)
                Error(_) -> send_and_shutdown(<<"err">>, handle_a, handle_b)
              }
            }
