...
import distribute/handshake_actor.{start_initiator_handshake}
...
pub fn initiator_actor_timeout_triggers_failure_test() {
  ...
  let on_success = fn(_node, _meta) { process.send(test_subject, Unexpected("success")) }
  let on_failure = fn(_node, reason) { process.send(test_subject, Failed(reason)) }
  let send_fn = fn(_bytes) { Nil }

  case start_initiator_handshake(local_hello, 50, send_fn, on_success, on_failure) {
    Ok(_subject) -> {
