...
pub fn crypto_concurrent_encrypt_decrypt_test() {
  ...
  let encrypted =
    list.map(messages, fn(m) {
      let assert Ok(enc) = { provider.encrypt }(handle, ctx, m)
      enc
    })

  // Decrypt all
  let decrypted =
    list.map(encrypted, fn(c) {
      let assert Ok(dec) = { provider.decrypt }(handle, ctx, c)
      dec
    })

  // Should match original (noop = identity)
  should.equal(decrypted, messages)

  // Metrics should track all operations
  let m = { provider.metrics }(handle)
  should.equal(m.encrypt_count, 5)
  should.equal(m.decrypt_count, 5)

  // Cleanup
  let _ = { provider.shutdown }(handle)
}

pub fn crypto_rekey_during_operations_test() {
  let name = "test_crypto_rekey_ops"
  let options = adapter.development_options(name)
  let provider = noop_adapter.new()
