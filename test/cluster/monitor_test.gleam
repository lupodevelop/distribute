...
pub fn monitor_lifecycle_test() {
  ...
  should.be_false(process.is_alive(
    process.subject_owner(monitor_subj) |> should.be_ok(),
  ))
}
...
pub fn monitor_multiple_subscribers_test() {
  let s1 = process.new_subject()
  let s2 = process.new_subject()

  let assert Ok(m1) = monitor.subscribe(s1)
  let assert Ok(m2) = monitor.subscribe(s2)

  let m1_pid = process.subject_owner(m1) |> should.be_ok()
  let m2_pid = process.subject_owner(m2) |> should.be_ok()

  // Both actors exist
  should.be_true(process.is_alive(m1_pid))
  should.be_true(process.is_alive(m2_pid))

  // Simulate event for both
  simulate_node_event(m1_pid, nodeup_atom(), "nodeA")
  simulate_node_event(m2_pid, nodeup_atom(), "nodeA")

  let assert Ok(NodeUp("nodeA")) = process.receive(s1, 500)
  let assert Ok(NodeUp("nodeA")) = process.receive(s2, 500)

  monitor.unsubscribe(m1)
  monitor.unsubscribe(m2)
}

pub fn monitor_owner_death_test() {
  let subj = process.new_subject()
  let assert Ok(m) = monitor.subscribe(subj)
  let m_pid = process.subject_owner(m) |> should.be_ok()

  should.be_true(process.is_alive(m_pid))

  // Stopping manually
  monitor.unsubscribe(m)
  process.sleep(50)
  should.be_false(process.is_alive(m_pid))
}
