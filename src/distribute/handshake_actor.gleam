...
import distribute/handshake_state as hs
...
pub fn start_initiator_handshake(
  ...
  let initial_state: Result(#(hs.HandshakeState, process.Subject(HandshakeMessage), Int), Nil) = Error(Nil)
  ...
  |> actor.on_message(fn(state, msg) {
    case state, msg {
      ...
      Ok(#(current, subject, timeout)), NetworkMessage(bytes) -> {
        handle_network_message(current, bytes, send_fn, subject, timeout, local, on_success, on_failure)
      }
      Ok(#(current, subject, timeout)), TimeoutMessage(_tag) -> {
        handle_timeout(current, send_fn, subject, timeout, local, on_success, on_failure)
      }
...
fn handle_network_message(
  ...
  on_success: fn(String, handshake.MemberMetadata) -> Nil,
  on_failure: fn(String, String) -> Nil,
) -> actor.Next(Result(#(hs.HandshakeState, process.Subject(HandshakeMessage), Int), Nil), HandshakeMessage) {
  case hs.initiator_handle_message(current, bytes) {
    ...
    Ok(hs.Received(None, hs.InitiatorState(..) as new_state)) -> actor.continue(Ok(#(new_state, subject, timeout_ms)))
    Ok(hs.Received(Some(out_bytes), hs.ResponderState(..) as new_state)) -> {
      send_fn(out_bytes)
      actor.continue(Ok(#(new_state, subject, timeout_ms)))
    }
    Ok(hs.Received(None, hs.ResponderState(..) as new_state)) -> actor.continue(Ok(#(new_state, subject, timeout_ms)))
...
fn schedule_if_waiting(state: hs.HandshakeState, subject: process.Subject(HandshakeMessage), timeout_ms: Int) -> Nil {
  case state {
...
fn schedule_timer_with_subject(subject: process.Subject(HandshakeMessage), tag: String, timeout_ms: Int) -> Nil {
